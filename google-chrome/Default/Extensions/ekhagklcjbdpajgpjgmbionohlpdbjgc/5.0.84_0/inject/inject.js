...
Zotero.Inject = new function () {
  ...
  this.init = function (force) {
    ...
    this.sessionDetails = {};
    _noteImgSrc = Zotero.isSafari ? `${safari.extension.baseURI}safari/` + "images/treeitem-note.png" : browser.extension.getURL('images/treeitem-note.png'); // wrap this in try/catch so that errors will reach logError
  ...
  this.notify = new function () {
    ...
    return function (text, buttons, timeout, tabStatus) {
      ...
      }.bind(this); // Sequentialize notification display
...
const isTestPage = Zotero.isBrowserExt && window.location.href.startsWith(browser.extension.getURL('test')); // don't try to scrape on hidden frames

...
(function () {
  ...
  Zotero.GoogleDocs.UI = {
    ...
    injectIntoDOM: async function () {
      Zotero.GoogleDocs.UI.menubutton = document.getElementById('docs-zotero-menubutton'); // The Zotero Menu
      ...
      ReactDOM.render(Zotero.GoogleDocs.UI.menu, Zotero.GoogleDocs.UI.menuDiv); // The Zotero button
      ...
      ReactDOM.render(Zotero.GoogleDocs.UI.citationEditor, Zotero.GoogleDocs.UI.citationEditorDiv); // Please wait screen
      ...
      }), Zotero.GoogleDocs.UI.pleaseWaitContainer); // Orphaned citation UI
      ...
      pasteTarget.addEventListener('paste', async e => {
        ...
        for (let data of docSlices) {
          for (let k in data.dsl_entitymap) {
            ...
            if (!rangeName) continue; // Ignore document data and bibliography style
        ...
        for (let i = 0; i < links.length; i++) {
          if (links[i] && links[i].lnks_link && links[i].lnks_link.ulnk_url.startsWith(Zotero.GoogleDocs.config.fieldURL)) {
            ...
            let linkRanges = ranges[linkStart].nrs_ei.cv.opValue // `&& key in keysToCode` is kinda strange
    ...
    addKeyboardShortcuts: async function () {
      let modifiers = await Zotero.Prefs.getAsync('shortcuts.cite'); // Store for access by Menu and Linkbubble widgets

      this.shortcut = Zotero.Utilities.kbEventToShortcutString(modifiers); // The toolbar button is managed by GDocs
    ...
    sendKeyboardEvent: async function (eventDescription, target) {
      if (!target) {
        target = document.querySelector('.docs-texteventtarget-iframe').contentDocument; // Keyboard events to texteventtarget (i.e. the pseudo-element in focus
    ...
    insertFootnote: async function () {
      ...
      await Zotero.GoogleDocs.UI.sendKeyboardEvent(insertFootnoteKbEvent); // Somehow the simulated footnote shortcut inserts an "F" at the start of the footnote.
    ...
    insertLink: async function (text, url) {
      ...
      var urlInput = document.getElementsByClassName('docs-link-urlinput-url')[0]; // New gdocs link input UI.
    ...
    closeInsertLinkPopup: async function (confirm = true) {
      ...
      if (confirm && urlInput.value) {
        ...
        let textEventTarget = document.querySelector('.docs-texteventtarget-iframe'); // Recently GDocs has made a change to footnote navigation where pressing [Escape]
    ...
    waitToSaveInsertion: async function () {
      ...
      var deferred = Zotero.Promise.defer(); // In case the new UI is being pushed out in phases we'll keep the old logic for now
      ...
      if (!newStyleSaveLabel) {
        ...
        if (!document.querySelector('.docs-icon-sync')) {
          ...
          await Zotero.Promise.delay(1000); // We wait an extra second and if there's still no sync indicator then:
  ...
  Zotero.GoogleDocs.UI.Menu = class extends React.Component {
    ...
    render() {
      ...
      let exportMenuItem = React.createElement(Zotero.GoogleDocs.UI.Menu.Item, {
      ...
      return React.createElement("div", {
        ...
        handleClick: this.props.execCommand.bind(this, 'addEditCitation', null),
        accel: Zotero.GoogleDocs.UI.shortcut
      }), React.createElement(Zotero.GoogleDocs.UI.Menu.Item, {
        label: "Add/edit bibliography",
        handleClick: this.props.execCommand.bind(this, 'addEditBibliography', null)
      }), React.createElement(Zotero.GoogleDocs.UI.Menu.Item, {
        label: "Document preferences...",
        handleClick: this.props.execCommand.bind(this, 'setDocPrefs', null)
      }), React.createElement(Zotero.GoogleDocs.UI.Menu.Item, {
        label: "Refresh",
        handleClick: this.props.execCommand.bind(this, 'refresh', null)
      }), exportMenuItem, React.createElement(Zotero.GoogleDocs.UI.Menu.Item, {
        label: "Unlink citations...",
        handleClick: this.props.execCommand.bind(this, 'removeCodes', null)
      }));
    }
  ...
  Zotero.GoogleDocs.UI.Menu.Item = class extends React.Component {
    ...
    render() {
      ...
      }, this.props.label), this.props.accel ? React.createElement("span", {
  ...
  Zotero.GoogleDocs.UI.LinkbubbleOverride = class extends React.Component {
    ...
    async componentDidMount() {
      ...
      Zotero.GoogleDocs.UI.lastLinkURL = url; // Check if on zotero field link
    ...
    render() {
      ...
      var top; // If we click away from the link and then on it again, google docs doesn't update
